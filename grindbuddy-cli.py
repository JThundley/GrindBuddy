...
class EventHandler():
    ...
    def __init__(self, config, logdir=None):
        self.config = config
    ...
    def isSectionSpeechTextOn(self, section, textorspeech):
        ...
        if textorspeech == 'speech' and not self.tts: # if we're checking speech but global speech is off...
            return False
        return self.config.getboolean(section, textorspeech, 'True')
    ...
    def printMissionsByReward(self):
        ...
        for num in range(len(missions)):
            print "\t\t%s%s: %s - %s" % (' ' * (strlen - len(format(missions[num][0], ','))), format(missions[num][0], ','), self.spaceship.missions[missions[num][1]]['DestinationSystem'], self.spaceship.missions[missions[num][1]]['DestinationStation'])
    def printMissionsByExpirationTime(self):
        ...
        for num in range(len(missions)):
            print "\t\t%s: %s - %s" % (self.convertToLocalTime(missions[num][0]).strftime('%m/%d/%Y %H:%M:%S %Z'), self.spaceship.missions[missions[num][1]][
                'DestinationSystem'], self.spaceship.missions[missions[num][1]]['DestinationStation'])
    ################################ EVENTS ##########################################################
    ...
    def CountTargetKills(self, journalentry):
        ...
        if journalentry['event'] == 'MissionAccepted' and 'Massacre' in journalentry['Name']: # if we accepted a massacre mission...
            ...
            try:
                self.target_kills[event['VictimFaction']][0] += 1
            except KeyError: # you just killed someone outside of your mission
                pass
            else: # announce it
                if self.target_kills[event['VictimFaction']][0] == self.target_kills[event['VictimFaction']][1]: # If we've reached the last kill
                    del self.target_kills[event['VictimFaction']] # remove this mission that's being tracked
                    return # and don't announce the last kill, the game will tell you "mission completed" which is enough.
                phrase = "That was kill number %s" % self.target_kills[event['VictimFaction']][0]
                if self.config.getboolean('Count Target Kills', 'total', 'True'):
                    phrase = "%s out of %s" % (phrase, self.target_kills[event['VictimFaction']][1])
